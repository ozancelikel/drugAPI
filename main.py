    ...
    for key, val in autocomplete_list.items():
        ...
        if interaction is not None:
            ...
            if interaction:
                ...
                graph_handler.write_relation(val, stored_atc, interaction[0]['interactionLevel'])
            else:
                interaction_list.append({"drug": key, "atc": val, "interaction_level":[
                {
                    "ingredient1": val,
                    "ingredient2": stored_atc,
                    "interactionLevel": "NO INTERACTION"
                }]})
    return interaction_list


@app.post("/recursive_autocomplete2", response_class=JSONResponse)
async def search_endpoint2(data: str = Form(...), stored: str = Form(...)):
    results = es_agg.get_autocomplete_results(data)
    autocomplete_list = {}
    for res in results["aggregations"]["auto_complete"]["buckets"]:
        atc_name = es_agg.get_atc_from_brand(res["key"])
